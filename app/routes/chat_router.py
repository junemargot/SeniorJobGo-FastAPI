...
from app.agents.multi_turn_agent import MultiTurnAgent
...
# LLM 초기화
llm = ChatOpenAI(model="gpt-4o-mini")  # 모델 이름을 지정하여 인스턴스 생성
agent = MultiTurnAgent(llm)  # 에이전트 초기화
...
async def chat(request: Request, chat_request: ChatRequest) -> ChatResponse:
    try:
        ...
        # 개인 정보가 포함된 경우 채용 정보 검색
        if user_profile.get("age"):
            logger.info(
                "[ChatRouter] 개인 정보가 포함되어 있습니다. 채용 정보 검색 시작."
            )
            response = await agent.generate_response(
                query="채용 정보가 궁금합니다.", user_profile=user_profile
            )
        else:
            # 일반 대화 처리
            response = await agent.generate_response(
                query=chat_request.user_message, user_profile=user_profile
            )
        ...
        bot_message = {"role": "bot", "content": response, "index": chat_index + 1}

        db.users.update_one(
            {"_id": ObjectId(_id)},
            {"$set": {"messages": [*legacy_messages, user_message, bot_message]}},
        )
