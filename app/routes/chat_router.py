...
async def chat(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"[ChatRouter] 쿠키에서 사용자 정보 조회 중 오류 발생 - 기본 응답 반환"
            )
            return ChatResponse(
        ...
        if (
            prev_message
            and "관련 훈련과정을" in prev_message
            and any(
                word in chat_request.user_message
                for word in ["저렴한", "저렴하게", "싼", "무료로", "비용이", "학비가"]
            )
        ):
            # 이전 검색 키워드 추출
        ...
        try:
            ...
            logger.error(
                "[ChatRouter] JobAdvisorAgent.chat 실행 중 에러", exc_info=True
            )
            raise chat_error

        # 가격 필터링이 필요한 경우
        if response.get("type") == "training" and any(
            word in chat_request.user_message
            for word in ["저렴한", "저렴하게", "싼", "무료로", "비용이", "학비가"]
        ):
            ...
            for course in courses:
                try:
                    cost = int(
                        course.get("cost", "0").replace(",", "").replace("원", "")
                    )
            ...
            if filtered_courses:
                ...
                response["message"] = (
                    f"30만원 이하의 저렴한 훈련과정 {len(filtered_courses)}개를 찾았습니다."
                )
            else:
                response["message"] = (
                    "죄송합니다. 조건에 맞는 저렴한 훈련과정을 찾지 못했습니다."
                )

        # 메시지 저장
        try:
            await add_chat_message(
                user, chat_request.user_message, response.get("message", "")
            )
...
async def search_training(
    ...
    try:
        ...
        if interests:
            ...
            message_keywords = [
                word
                for word in chat_request.user_message.split()
