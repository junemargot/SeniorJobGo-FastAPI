...
async def search_trainings(
    search_params: TrainingSearchRequest, training_advisor=Depends(get_training_advisor)
):
    ...
    try:
        ...
        result = await training_advisor.search_training_courses(
            query=search_query.rstrip(", "), user_profile=user_profile
        )
        ...
        return {
            "message": result.get(
                "message",
                f"{location or '전국'} 지역의 훈련과정을 {len(result.get('trainingCourses', []))}건 찾았습니다.",
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"훈련과정 검색 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_locations():
    ...
    try:
        ...
        logger.error(
            f"[TrainingRouter] 지역 정보 조회 중 오류: {str(e)}", exc_info=True
        )
        raise HTTPException(
...
async def get_interests():
    ...
    try:
        ...
        logger.error(
            f"[TrainingRouter] 관심 분야 정보 조회 중 오류: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500,
