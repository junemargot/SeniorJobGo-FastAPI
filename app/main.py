...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        llm = ChatOpenAI(model_name="gpt-4o-mini", temperature=0.7, request_timeout=30)
        ...
        # ResumeAdvisorAgent 초기화 추가
        app.state.resume_advisor_agent = ResumeAdvisorAgent(llm=llm)
    ...
    try:
        ...
        response = app.state.llm.invoke(
            EXTRACT_INFO_PROMPT.format(
                user_query=user_message, chat_history=chat_history
            )
        )
